...
# 1. 创建HDFS目录
create_hdfs_dir("/warehouse/gd7/dim/dim_product")
...
STORED AS PARQUET
LOCATION '/warehouse/gd7/dim/dim_product'
...
dim_product.write.mode("overwrite") \
    .partitionBy("dt") \
    .parquet("/warehouse/gd7/dim/dim_product")
...
# 1. 创建HDFS目录
create_hdfs_dir("/warehouse/gd7/dim/dim_store")
...
PARTITIONED BY (dt string COMMENT '统计日期')
STORED AS PARQUET
LOCATION '/warehouse/gd7/dim/dim_store'
TBLPROPERTIES ('parquet.compress' = 'SNAPPY')
""")
...
store_ids_from_orders = spark.sql("""
    SELECT DISTINCT store_id 
    FROM ods_order_info 
    WHERE dt = '20250801'
""")
...
# 合并所有店铺ID
all_store_ids = store_ids_from_behavior.union(store_ids_from_orders).distinct()
...
# 从订单信息中提取店铺相关信息
store_info_from_orders = spark.sql("""
    SELECT 
        store_id,
        FIRST(create_time) as create_time,
        FIRST(update_time) as update_time
...
# 合并店铺信息
all_store_info = store_info_from_behavior.union(store_info_from_orders).groupBy("store_id").agg(
    F.max("create_time").alias("create_time"),
    F.max("update_time").alias("update_time")
)

# 创建店铺维度数据，直接使用ODS层的店铺ID
dim_store = all_store_info.join(all_store_ids, "store_id").select(
    F.col("store_id"),
...
dim_store.write.mode("overwrite") \
    .partitionBy("dt") \
    .parquet("/warehouse/gd7/dim/dim_store")
...
# 1. 创建HDFS目录
create_hdfs_dir("/warehouse/gd7/dim/dim_user")
...
PARTITIONED BY (dt string COMMENT '统计日期')
STORED AS PARQUET
LOCATION '/warehouse/gd7/dim/dim_user'
TBLPROPERTIES ('parquet.compress' = 'SNAPPY')
""")
...
all_user_info = user_info_from_behavior.union(user_info_from_orders).groupBy("user_id").agg(
    ...
    F.max("last_login_time").alias("last_login_time")
)

# 创建用户维度数据，直接使用ODS层的用户ID
...
dim_user.write.mode("overwrite") \
    .partitionBy("dt") \
    .parquet("/warehouse/gd7/dim/dim_user")
...
try:
    ...
    print("店铺维度表样本数据:")
    spark.sql("SELECT * FROM dim_store WHERE dt = '20250801' LIMIT 5").show(truncate=False)
