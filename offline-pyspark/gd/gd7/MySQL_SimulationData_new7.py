...
# 真实的支付方式
PAYMENT_WAYS = ['支付宝', '微信支付', '银行卡', '货到付款', 'Apple Pay']
...
VALUE_GENERATION_RULES = {
    ...
    'password': lambda field, context: f"pwd{random.randint(100000, 999999)}",
...
def generate_realistic_name(field, context):
    ...
    if 'user' in field_name or 'login' in field_name or 'nick' in field_name:
        return random.choice(REAL_USER_NAMES) + str(random.randint(1, 999))

    # 商品相关名称
    elif 'sku' in field_name or 'spu' in field_name or 'product' in field_name:
        return random.choice(REAL_PRODUCT_NAMES) + " " + str(random.randint(1, 999))
    ...
    # 通用名称
        ...
        return f"{random.choice(REAL_USER_NAMES)}_{random.randint(1, 99999)}"

# 生成真实的邮箱
def generate_realistic_email(field, context):
    username = random.choice(REAL_USER_NAMES).replace(' ', '').lower() + str(random.randint(1, 999))
...
def generate_realistic_general_value(field, context):
    ...
    if 'img' in field_name or 'image' in field_name:
        value = random.choice(REAL_IMAGE_URLS)
    elif 'passwd' in field_name or 'password' in field_name:
        value = f"pwd{random.randint(100000, 999999)}"
    elif 'level' in field_name:
        value = random.choice(USER_LEVELS)
    elif 'way' in field_name:
        value = random.choice(PAYMENT_WAYS)
    elif 'comment' in field_name or 'remark' in field_name:
        value = random.choice(REAL_PRODUCT_DESCRIPTIONS)
    elif 'head' in field_name:
        value = random.choice(REAL_IMAGE_URLS)
    elif 'url' in field_name:
        value = random.choice(REAL_IMAGE_URLS)
    elif 'code' in field_name:
        value = f"CODE{random.randint(10000, 99999)}"
    elif 'trade' in field_name:
        value = f"TRADE{random.randint(100000000, 999999999)}"
    elif 'tracking' in field_name:
        value = f"TRACK{random.randint(100000000, 999999999)}"
    else:
        # 根据字段类型生成值
        if 'int' in field_type:
            ...
            # 默认生成有意义的字符串
            value = random.choice(REAL_USER_NAMES) + "_" + str(random.randint(1, 9999))
...
def generate_general_value(field, context):
    ...
    if 'int' in field_type:
        ...
        # 默认生成字符串值
        return random.choice(REAL_USER_NAMES) + "_" + str(random.randint(1, 9999))
