...
# 读取DWD层各事实表数据
dwd_view = spark.sql("SELECT * FROM dwd_product_view WHERE dt = '20250801'")
dwd_collect = spark.sql("SELECT * FROM dwd_product_collect WHERE dt = '20250801'")
dwd_cart = spark.sql("SELECT * FROM dwd_product_cart WHERE dt = '20250801'")
dwd_order = spark.sql("SELECT * FROM dwd_product_order WHERE dt = '20250801'")
...
collect_agg = dwd_collect.groupBy("product_id").agg(
    F.sum("collector_count").alias("collect_count")
)
...
dws_product_efficiency = view_agg.alias("v") \
    ...
    F.coalesce(F.col("view_count"), F.lit(0)).alias("view_count"),
    F.coalesce(F.col("visitor_count"), F.lit(0)).alias("visitor_count"),
    F.coalesce(F.col("collect_count"), F.lit(0)).alias("collect_count"),
    F.coalesce(F.col("cart_quantity"), F.lit(0)).alias("cart_quantity"),
    F.coalesce(F.col("cart_user_count"), F.lit(0)).alias("cart_user_count"),
    F.coalesce(F.col("order_quantity"), F.lit(0)).alias("order_quantity"),
    F.coalesce(F.col("order_amount"), F.lit(0.0)).alias("order_amount"),
    F.coalesce(F.col("order_user_count"), F.lit(0)).alias("order_user_count"),
    F.coalesce(F.col("paid_quantity"), F.lit(0)).alias("paid_quantity"),
    F.coalesce(F.col("paid_amount"), F.lit(0.0)).alias("paid_amount"),
    F.coalesce(F.col("paid_user_count"), F.lit(0)).alias("paid_user_count"),
...
# 读取商品效率表和商品维度表数据
efficiency_data = spark.sql("SELECT * FROM dws_product_efficiency WHERE dt = '20250801'")
product_dim = spark.sql("SELECT product_id, category_id, price FROM dim_product WHERE dt = '20250801'")
...
print("验证DWS层数据:")
try:
    efficiency_count = spark.sql("SELECT COUNT(*) as count FROM dws_product_efficiency WHERE dt = '20250801'").collect()[0]['count']
    range_count = spark.sql("SELECT COUNT(*) as count FROM dws_product_range_analysis WHERE dt = '20250801'").collect()[0]['count']
    print(f"商品效率汇总表记录数: {efficiency_count}")
    print(f"商品范围分析表记录数: {range_count}")

    # 展示部分数据样本
    print("商品效率汇总表样本数据:")
    spark.sql("SELECT * FROM dws_product_efficiency WHERE dt = '20250801' LIMIT 5").show(truncate=False)

    print("商品范围分析表样本数据:")
    spark.sql("SELECT * FROM dws_product_range_analysis WHERE dt = '20250801' LIMIT 5").show(truncate=False)
